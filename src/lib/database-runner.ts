...
import pg from "pg";
import { PATTERNS, getPaths, type SqlFile } from "./constants";
...
export async function runSqlFile(filePath?: string): Promise<void> {
	...
	if (!connectionString) {
		...
		console.error(
			"Please set SUPABASE_CONNECTION_STRING environment variable.",
		);
		console.error("\nExample:");
		console.error(
			'  export SUPABASE_CONNECTION_STRING="postgresql://user:password@localhost:5432/dbname"',
		);
		process.exit(1);
	}
